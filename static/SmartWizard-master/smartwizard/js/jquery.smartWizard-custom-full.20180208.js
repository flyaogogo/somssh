...
;(function ($, window, document, undefined) {
    ...
    $.extend(SmartWizard.prototype, {
        ...
        _showRelease: function () {
            ...
            // Find the next not disabled step
            var fi = this.steps.length;

            this._loadStepRelease(si, fi);
            return true;
        },
        ...
        _loadStepRelease: function (idx, fdx) {
            var mi = this;
            // Get current step elements
            // var curTab = this.steps.eq(this.current_index);
            // // Get the direction of step navigation
            ...
            var ajaxSettings = function (sub_step) {
                $.ajax({
                    ...
                    beforeSend: function (xhr, settings) {
                        ...
                        //$("#panel-75693-" + idx + " .badge").html('');
                        $("#panel-75693-" + idx).html('');
                    },
                    error: function (jqXHR, status, message) {
                        elm.parent('li').removeClass('loading');
                        $.error(message);
                    ...
                    success: function (res) {
                        ...
                        if(res.code === 999){
                            alert("发布任务执行中，请勿重复执行...");
                            //return false;
                            window.location.href = '/manage/job/list/';
                        }
                        //if (res && res.length > 0) {
                        //elm.data('has-content', true);
                        $("#pub_text").html("发布结果");
                        ...
                        $.each(res.result, function (i, item) {
                            ...
                            html = html + '<div class="panel panel-default"><div class="panel-heading">' +
                                '<a class="panel-title collapsed" data-toggle="collapse" data-parent="#panel-75693-' + idx +
                                '" href="#panel-' + idx + fnum + '"><div>' + i + '<span class="badge pull-right ' + spancss +
                                '"><i class="fa ' + icss + '"></i></span></div></a></div>' +
                                '<div id="panel-' + idx + fnum + '" class="panel-collapse collapse"><div class="panel-body step' + idx +
                                '"><p>Retcode: ' + item['retcode'] + '<br />JobID: ' + item['jid'] + '<br />Stderr: <br />' + item['stderr'] + '<br />Stdout: <br />' +
                                item['stdout'] + '<br /></p></div></div></div>';
                            fnum = fnum + 1;
                        });

                        if (res.code === 0) {
                            var rst_html = '<div class="btn-group" data-toggle="buttons" style="padding-left:0;">' +
                                '<a href="#" onclick="result_retry(' + idx + ');" type="button" class="btn btn-sm btn-default" name="result_deal"> 失败重试</a>' +
                                '<a href="#" onclick="result_mark();" type="button" class="btn btn-sm btn-default" name="result_deal"> 标记成功</a></div>';
                            var rst_html = '';
                            $("#result_deal").html(rst_html);
                        } else if (res.code === 1 && fdx > idx) {
                                //$('.sw-btn-rel').html("完成");
                                $('.sw-btn-rel').addClass("disabled");
                        } else {
                            $('.sw-btn-rel').removeClass("disabled");
        ...
        _loadStepRollback: function (idx) {
            ...
            if (logid === '') {
                $("#panel-75693-" + idx).html("");
                $("#result_deal").html("获取不到JLog ID，请到《任务记录》列表进行回退操作");
                return false;
            }
            var ajaxSettings = $.extend(true, {}, {
                url: rollbackURL,
                type: "POST",
                data: {step_number: idx, logid: logid},
                dataType: "json",
                beforeSend: function (xhr, settings) {
                    if (!csrfSafeMethod(settings.type) && !this.crossDomain) {
                        xhr.setRequestHeader("X-CSRFToken", csrftoken);
                ...
                success: function (res) {
                    ...
                    $.each(res, function (i, item) {
                        ...
                        html = html + '<div class="panel panel-default"><div class="panel-heading">' +
                            '<a class="panel-title collapsed" data-toggle="collapse" data-parent="#panel-75693-' + idx +
                            '" href="#panel-' + idx + fnum + '">' + i + '<span class="badge pull-right ' + spancss +
                            '"><i class="fa ' + icss + '"></i></span></a></div>' +
                            '<div id="panel-' + idx + fnum + '" class="panel-collapse collapse"><div class="panel-body step' + idx +
                            '"><p>Retcode: ' + item['retcode'] + '<br />Stderr: <br />' + item['stderr'] + '<br />Stdout: <br />' +
                            item['stdout'] + '<br /></p></div></div></div>';
                        fnum = fnum + 1;
                    });
